...
/// Host functions required for clover runtime and Substrate node.
...
pub fn create_extrinsic(
    ...
    function: impl Into<clover_runtime::RuntimeCall>,
    nonce: Option<u32>,
) -> clover_runtime::UncheckedExtrinsic {
    ...
    let period = clover_runtime::BlockHashCount::get()
        .checked_next_power_of_two()
        .map(|c| c / 2)
        .unwrap_or(2) as u64;
    let tip = 0;
    let extra: clover_runtime::SignedExtra = (
        frame_system::CheckSpecVersion::<clover_runtime::Runtime>::new(),
        frame_system::CheckTxVersion::<clover_runtime::Runtime>::new(),
        frame_system::CheckGenesis::<clover_runtime::Runtime>::new(),
        frame_system::CheckEra::<clover_runtime::Runtime>::from(generic::Era::mortal(
            period,
            best_block.saturated_into(),
        )),
        frame_system::CheckNonce::<clover_runtime::Runtime>::from(nonce),
        frame_system::CheckWeight::<clover_runtime::Runtime>::new(),
        pallet_transaction_payment::ChargeTransactionPayment::<clover_runtime::Runtime>::from(0),
    );

    let raw_payload = clover_runtime::SignedPayload::from_raw(
        function.clone(),
        extra.clone(),
        (
            clover_runtime::VERSION.spec_version,
            clover_runtime::VERSION.transaction_version,
            genesis_hash,
            best_hash,
            (),
            (),
    ...
    clover_runtime::UncheckedExtrinsic::new_signed(
        function,
        sp_runtime::AccountId32::from(sender.public()).into(),
        clover_runtime::Signature::Sr25519(signature),
        extra,
    )
}
